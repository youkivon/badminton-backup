...
# 帧目录：优先用 args.frames_dir，否则按球员分开
if args.frames_dir:
...
def extract_smart_frames(video_path, frames_dir, min_gap_sec=1):
    ...
    tmp_all = "/tmp/bad_shots/all_frames"
...
def load_or_run_analysis(frames_dir):
    """加载已有分析数据，或重新分析"""
    # --skip_analysis 时缓存不存在，直接报错
    ...
    if use_v2:
        # V2 Stage1已过滤非挥拍帧，直接用
        # 过滤：只保留Stage1检测到挥拍的帧（hitter_side != "Unknown"）+ unable帧（VLM分析过）
        valid_shots = [
            r for r in results
...
shots = load_or_run_analysis(SESSION_FRAMES_DIR)
...
report_data = {
    ...
    "shots": shots,
    "frames_dir": SESSION_FRAMES_DIR,   # ← 传给 report_generator 用
}
...
# ── Step 5: 推送微信报告 ──────────────────────────────────
def send_wechat_report(pdf_path: str, player_name: str, shots_count: int, avg_quality: float):
...
send_wechat_report(desktop_path, PLAYER_NAME, len(shots), avg_q)
